...
// 再取并发送 takeHandshakeOutput()：可能包含 client Finished 或 close_notify。
...
3. Handshake 级别字节交给 `receiveHandshake()`；固定验证 EE → Certificate → CV → Finished，允许同级分片/合并。
4. `authenticated` 后取 `clientFinished()` 并在 Handshake 级别发送；此时才能取得
